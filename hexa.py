...
def convert():
    """Converts the given integer into hexadecimal"""
    try:
        integer = int(entry_1.get())
        if integer < 0:
            messagebox.showerror("Error", "Masukan Integer Positif")
        else:
            hexadecimal = hex(integer).replace("0x", "")
            canvas.itemconfig(text, text=hexadecimal)
    except ValueError:
        messagebox.showerror("Error", "Mohon masukan integer")
